...
const Login: FC = () => {
	...
	const verifyOtpHandler = async (e: FormEvent) => {
		...
		if (verifyOtpResponse.success) {
			const updatedUserPassword = (await patchRequest("/users/reset-password", {
				email,
				newPassword,
			})) as ApiResponse;
	...
	return (
		<section className="login-section">
			...
			<Modal open={modal} onClose={() => setModal(false)}>
				<form className="login-section__modal" onSubmit={sendOtpHandler}>
			...
			<div className="login-section__container">
				...
				<form className="login-section__form" onSubmit={submitHandler}>
					...
					<p className="login-section__link">
						Don&rsquo;t have account <Link to="/signup">Signup</Link>
					</p>

					<p className="login-section__link" onClick={() => setModal(true)}>
